...
`;
function renderCarouselSlides(items, wrapper) {
...
function renderPopularMovies(movies, container) {
    const fragment = document.createDocumentFragment();
    movies.forEach((movie) => {
        ...
        // 設置上映狀態標籤
        const statusBadge = movieCard.querySelector(".movie-status");
        const statusType = movie.statusType || "current";
...
function renderLatestNews(news, container) {
    const fragment = document.createDocumentFragment();
    news.forEach((item) => {
        ...
        // 設置分類標籤
        const category = newsCard.querySelector(".news-category");
...
// 初始化輪播 - 使用統一的電影數據
const carouselSlide = document.querySelector(HERO_CAROUSEL_SLIDE_SELECTOR);
if (carouselSlide) {
    renderCarouselSlides(moviesData, carouselSlide);
}
const carousel = new Carousel(HERO_CAROUSEL_CONTAINER_SELECTOR, HERO_CAROUSEL_SLIDE_SELECTOR);
carousel.autoPlay(5000);
// 初始化熱門電影區域 - 取前4部電影作為熱門電影
const popularMoviesContainer = document.querySelector(POPULAR_MOVIES_CONTAINER_SELECTOR);
...
// 初始化最新消息區域
const newsContainer = document.querySelector(NEWS_CONTAINER_SELECTOR);
if (newsContainer) {
